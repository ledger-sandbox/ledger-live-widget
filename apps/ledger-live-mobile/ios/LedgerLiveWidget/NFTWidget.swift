...
struct Provider1: AppIntentTimelineProvider {
    ...
    func placeholder(in context: Context) -> SimpleEntry1 {
    let nftData = nfts[.nft1]!
    return SimpleEntry1(date: Date(), nftURL: URL(string: nftData.imageURL), nftName: nftData.name, configuration: SelectNFTIntent())
}

    func snapshot(for configuration: SelectNFTIntent, in context: Context) async -> SimpleEntry1 {
        let nftData = nfts[configuration.selectedNFT ?? .nft1] ?? nfts[.nft1]!
        return SimpleEntry1(date: Date(), nftURL: URL(string: nftData.imageURL), nftName: nftData.name, configuration: configuration)
    }

    func timeline(for configuration: SelectNFTIntent, in context: Context) async -> Timeline<SimpleEntry1> {
        let nftData = nfts[configuration.selectedNFT ?? .nft1] ?? nfts[.nft1]!
        let entry = SimpleEntry1(date: Date(), nftURL: URL(string: nftData.imageURL), nftName: nftData.name, configuration: configuration)
        return Timeline(entries: [entry], policy: .atEnd)
    }
}

struct SimpleEntry1: TimelineEntry {
    let date: Date
    let nftURL: URL?
    let nftName: String
...
struct NFTWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        AppIntentConfiguration(kind: kind, intent: SelectNFTIntent.self, provider: Provider1()) { entry in
